...
gm.prototype.identify = function(callback){
    ...
    var bigthumb = 'ffmpeg -y -i ' + self.source + '-ss 0 -vframes 1 bigt/'+self.source.slice(0,self.source.lastIndexOf('.'))+'.jpg'
    self._exec(bigthumb, function(err, stdout, stderr) {});
    self.source = self.source.slice(0,self.source.lastIndexOf('/')) + '/bigt'+   self.source.slice(self.source.lastIndexOf('/'));
