...
// this parses the info from ffmpeg and returns an array.
var identify = function(src, what){
    var get = ffmpeg('ping', src, '');
    var stringout = '' + get[1]; // coerce into string for sure
    var pieces = stringout.replace(/=/g,"\n").split('\n'); //make an array 
    switch(what){
    case 'width':
	    return  parseInt(pieces[pieces.indexOf("width")+1]);
        break;
    case 'height':
        return parseInt(pieces[pieces.indexOf("height")+1]);
        break;
    case 'geo':
        return { width: parseInt(pieces[pieces.indexOf("width")+1]), height: parseInt(pieces[pieces.indexOf("height")+1]) };
        break;
    case 'format':
        return pieces[pieces.indexOf("codec_type")+1].toLowerCase();
        break;
    case 'filesize':
    	return parseInt(pieces[pieces.indexOf("size")+1]);
        break;
    case 'error':
    	return get[0];
        break;
